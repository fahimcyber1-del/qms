...
import { 
  ...
  TrendingUp, Flag, Activity, Download, LayoutGrid, Edit2
} from 'lucide-react';
...
export function QualityGoalsForm({ onNavigate, params }: Props) {
  ...
  const handleExport = () => {
    openExportPreview({
      ...
      moduleId: 'quality_goal_record',
      recordId: formData.id,
      fileName: `Quality_Goal_${formData.id}`,
      fields: [
        ...
        { label: 'Target End Date', value: formData.endDate },
        { label: 'Target Value', value: `${formData.targetValue} ${formData.uom}` },
        { label: 'Actual Value', value: `${formData.actualValue} ${formData.uom}` },
        { label: 'Progress (%)', value: `${((formData.actualValue / formData.targetValue) * 100).toFixed(1)}%` },
          ...
          {isReadOnly ? (
             <>
               <button type="button" onClick={handleExport} className="btn btn-ghost border border-border-main flex items-center gap-2">
                  <Download className="w-4 h-4" /> Export Goal
               </button>
