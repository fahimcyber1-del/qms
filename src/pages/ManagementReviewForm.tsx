...
export function ManagementReviewForm({ onNavigate, params }: Props) {
  ...
  const handleExport = () => {
    openExportPreview({
      moduleName: 'Management Review Minutes',
      moduleId: 'mr_detail',
      recordId: formData.id,
      fileName: `MR_${formData.id}_Minutes`,
      layout: 'executive',
      fields: [
        { label: 'Review Title', value: formData.reviewTitle },
        { label: 'Review Type', value: formData.reviewType },
        { label: 'Meeting Date', value: formData.date },
        { label: 'Chairperson', value: formData.chairperson },
        { label: 'Minutes Taker', value: formData.responsiblePerson },
        { label: 'Attendees', value: formData.attendees, fullWidth: true },
        { label: 'Review Inputs', value: formData.inputItems, fullWidth: true },
        { label: 'Decisions & Outputs', value: formData.outputDecisions, fullWidth: true },
        { label: 'Action Items', value: formData.actionItems, fullWidth: true },
        { label: 'Resource Needs', value: formData.resourceNeeds, fullWidth: true },
    ...
    <form className="p-4 md:p-8 space-y-6 max-w-5xl mx-auto" onSubmit={handleSave}>
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-2">
        ...
        <div className="flex items-center gap-3">
          {isReadOnly ? (
            <>
              <button type="button" onClick={() => onNavigate('management-review-form', { mode: 'edit', data: formData })} className="btn btn-ghost border border-border-main flex items-center gap-2">
              ...
              <button type="button" onClick={handleExport} className="btn btn-ghost border border-border-main flex items-center gap-2">
                <Download className="w-4 h-4" /> Export Minutes
              </button>
            </>
          ) : (
            <>
              <button type="button" onClick={() => onNavigate('management-review')} className="btn btn-ghost px-6">Cancel</button>
