...
import { 
  ArrowLeft, Save, X, Wrench, Building, User, Calendar, 
...
export function EquipmentMaintenanceForm({ onNavigate, params }: Props) {
  const mode = params?.mode || 'create';
  const initialData = params?.data || {};

  const [formData, setFormData] = useState({
    id: initialData.id || `MTC-${Date.now()}`,
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === 'create') {
        await getTable('equipmentMaintenance').add({
          ...formData,
          createdAt: new Date().toISOString()
        });
      } else {
        await getTable('equipmentMaintenance').put(formData);
      }
    ...
    <form className="p-4 md:p-8 space-y-6 max-w-5xl mx-auto" onSubmit={handleSave}>
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-2">
        <div className="flex items-center gap-4">
          <button type="button" onClick={() => onNavigate('equipment-maintenance')} className="w-10 h-10 rounded-xl bg-bg-1 border border-border-main flex items-center justify-center hover:bg-bg-2 transition-all">
            <ArrowLeft className="w-5 h-5 text-text-1" />
          </button>
          <div>
            <h2 className="text-2xl font-bold text-text-1">
              {mode === 'create' ? 'Log Maintenance Activity' : mode === 'edit' ? 'Update Service Record' : 'Equipment Service Details'}
            </h2>
      ...
      <Section title="Asset Identification" icon={Wrench} number="01">
        <Field label="Equipment / Machine Name" required span2>
          <input 
            type="text" required disabled={isReadOnly}
            className={inputClass} 
            value={formData.equipmentName} 
            onChange={e => setFormData(p => ({ ...p, equipmentName: e.target.value }))}
            placeholder="e.g., Juki Single Needle Sewing Machine - Line B"
          />
        </Field>
        <Field label="Serial / Asset Number" required>
          <input 
            type="text" required disabled={isReadOnly}
            className={inputClass} 
            value={formData.serialNo} 
            onChange={e => setFormData(p => ({ ...p, serialNo: e.target.value }))}
            placeholder="e.g., SN-889900-MTC"
          />
        </Field>
        <Field label="Factory Section" required>
          <select 
            disabled={isReadOnly}
            className={inputClass}
      ...
      <Section title="Service History" icon={Hammer} number="02">
        <Field label="Date of Maintenance" required>
          <input 
        ...
        <Field label="Next Service Due" required>
          <input 
            type="date" required disabled={isReadOnly}
            className={inputClass} 
        ...
        <Field label="Maintenance Type" required>
          ...
          >
            ...
            <option>Calibration Linked</option>
      ...
      <Section title="Work & Results" icon={Gauge} number="03">
        <Field label="Work Performed" span2>
           <textarea 
            disabled={isReadOnly}
            className={`${inputClass} min-h-[100px] bg-accent/5`} 
            value={formData.workDone} 
            onChange={e => setFormData(p => ({ ...p, workDone: e.target.value }))}
            placeholder="Describe the maintenance steps taken..."
          />
        </Field>
        <Field label="Parts Replaced">
           <input 
            ...
            placeholder="e.g., Oil Filter, Belt"
          />
        </Field>
        <Field label="Service Engineer" required>
          <input 
            type="text" required disabled={isReadOnly}
            className={inputClass} 
