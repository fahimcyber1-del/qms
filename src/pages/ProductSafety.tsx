...
export function ProductSafety({ onNavigate }: Props) {
  ...
  useEffect(() => {
    const load = async () => {
      const data = await getTable('productSafety').toArray();
  ...
  const filteredRecords = useMemo(() => {
    return records.filter(r => {
      const matchesSearch = 
        ...
        r.productType.toLowerCase().includes(searchQuery.toLowerCase()) ||
  ...
  const stats = useMemo(() => {
    return {
      ...
      warning: records.filter(r => r.status === 'Non-Compliant').length,
      underReview: records.filter(r => r.status === 'Under Assessment' || r.status === 'Open').length
    };
  }, [records]);

  const handleDelete = async (id: string) => {
    if (window.confirm("Are you sure?")) {
      await getTable('productSafety').delete(id);
  ...
  const exportExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "Safety Assessments");
    ...
    <motion.div className="p-4 md:p-8 space-y-8" variants={containerVariants} initial="hidden" animate="show">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-text-2 text-base mt-2">Hazard assessments, compliance checks, and legal safety standards.</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
        {[
          { label: 'Total Assessments', value: stats.total, icon: ShieldCheck, color: 'text-blue-500', bg: 'bg-blue-500/10' },
          { label: 'Fully Compliant', value: stats.compliant, icon: CheckCircle2, color: 'text-green-500', bg: 'bg-green-500/10' },
          { label: 'Non-Compliant', value: stats.warning, icon: AlertTriangle, color: 'text-red-500', bg: 'bg-red-500/10' },
          { label: 'Under Review', value: stats.underReview, icon: Clock, color: 'text-amber-500', bg: 'bg-amber-500/10' },
        ].map((stat, idx) => (
      ...
      <motion.div variants={itemVariants} className="flex flex-wrap items-center gap-4 bg-bg-1 p-3 rounded-2xl border border-border-main shadow-sm">
        <div className="relative flex-1 min-w-[250px]">
          ...
          <input 
            type="text" 
            placeholder="Search assessments, products, buyers..." 
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-bg-2/50 border-b border-border-main text-[10px] uppercase tracking-widest text-text-2 font-black">
                <th className="p-4 pl-6">Assessment Context</th>
              ...
              {filteredRecords.map(r => (
                <tr key={r.id} className="hover:bg-bg-2/60 transition-all duration-200 group">
                  <td className="p-4 pl-6">
                    <div className="font-bold text-text-1 text-sm truncate max-w-[250px]">{r.assessmentTitle}</div>
                    <div className="text-[11px] text-text-3 mt-1 font-mono uppercase tracking-tight">{r.id} • {r.department}</div>
                  </td>
                  <td className="p-4 text-sm font-semibold text-text-1">
                    {r.productType}
                  </td>
                  <td className="p-4">
                    <div className="flex items-center gap-2">
                       <Building className="w-3.5 h-3.5 text-accent opacity-50" />
                  ...
                  <td className="p-4 text-center">
                    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-[9px] font-black uppercase tracking-tighter border ${
                      r.status === 'Compliant' ? 'bg-green-500/10 text-green-500 border-green-500/20' : 
                      r.status === 'Non-Compliant' ? 'bg-red-500/10 text-red-500 border-red-500/20' : 
                  ...
                  <td className="p-4 pr-6 text-right">
                    <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button className="w-8 h-8 rounded-lg flex items-center justify-center hover:bg-accent/10 hover:text-accent text-text-2" onClick={() => onNavigate('product-safety-form', { mode: 'view', data: r })}>
