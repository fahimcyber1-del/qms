...
export function ContinuousImprovement({ onNavigate }: Props) {
  const [records, setRecords] = useState<CIRecord[]>([]);
  ...
  useEffect(() => {
    const load = async () => {
      const data = await getTable('continuousImprovement').toArray();
  ...
  const filteredRecords = useMemo(() => {
    return records.filter(r => {
      const matchesSearch = 
        r.improvementTitle.toLowerCase().includes(searchQuery.toLowerCase()) ||
  ...
  const handleDelete = async (id: string) => {
    if (window.confirm("Are you sure?")) {
      await getTable('continuousImprovement').delete(id);
  ...
  return (
    <motion.div className="p-4 md:p-8 space-y-8" variants={containerVariants} initial="hidden" animate="show">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
        {[
          { label: 'Total Projects', value: stats.total, icon: Rocket, color: 'text-blue-500', bg: 'bg-blue-500/10' },
          { label: 'Completed', value: stats.completed, icon: CheckCircle2, color: 'text-green-500', bg: 'bg-green-500/10' },
          { label: 'In Progress', value: stats.inProgress, icon: Sparkles, color: 'text-purple-main', bg: 'bg-purple-main/10' },
          { label: 'Proposed', value: stats.proposed, icon: Lightbulb, color: 'text-amber-500', bg: 'bg-amber-500/10' },
        ].map((stat, idx) => (
      ...
      <motion.div variants={itemVariants} className="flex flex-wrap items-center gap-4 bg-bg-1 p-3 rounded-2xl border border-border-main shadow-sm">
        ...
        <select className="bg-bg-2 border-none rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-accent outline-none text-text-1" value={filterCategory} onChange={(e) => setFilterCategory(e.target.value)}>
          ...
          <option value="Efficiency">Productivity</option>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-bg-2/50 border-b border-border-main text-[10px] uppercase tracking-widest text-text-2 font-black">
                <th className="p-4 pl-6">Initiative Name</th>
                <th className="p-4">Category & Department</th>
                <th className="p-4">Owner</th>
                <th className="p-4 text-center">Target Completion</th>
              ...
              {filteredRecords.map(r => (
                <tr key={r.id} className="hover:bg-bg-2/60 transition-all duration-200 group">
                  <td className="p-4 pl-6">
                    <div className="font-bold text-text-1 text-sm">{r.improvementTitle}</div>
                  ...
                  <td className="p-4">
                    <div className="flex items-center gap-2">
                       <div className="w-7 h-7 rounded-full bg-accent/10 border border-accent/20 flex items-center justify-center text-[10px] font-black text-accent uppercase">
                          {r.responsiblePerson.substring(0, 2)}
                       </div>
                       <span className="text-sm font-semibold text-text-1">{r.responsiblePerson}</span>
                    </div>
                  </td>
                  <td className="p-4 text-center text-xs font-semibold text-text-2 uppercase">
                    {new Date(r.targetDate).toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric'})}
                  </td>
                  <td className="p-4 text-center">
                    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-[9px] font-black uppercase tracking-tighter border ${
                      ...
                      r.status === 'In Progress' ? 'bg-purple-main/10 text-purple-main border-purple-main/20' : 
                  ...
                  <td className="p-4 pr-6 text-right">
                    <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button className="w-8 h-8 rounded-lg flex items-center justify-center hover:bg-accent/10 hover:text-accent text-text-2" onClick={() => onNavigate('continuous-improvement-form', { mode: 'view', data: r })}>
