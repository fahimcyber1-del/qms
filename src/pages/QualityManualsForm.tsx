...
import { 
  ...
  ScrollText, CheckCircle2, AlertCircle, Info, Paperclip, Plus, Trash2, FileText, Award, ShieldCheck, Bookmark, Download, Edit2
} from 'lucide-react';
...
export function QualityManualsForm({ onNavigate, params }: Props) {
  ...
  const handleExport = () => {
    openExportPreview({
      moduleName: 'Quality Manual Specification',
      moduleId: 'manual_detail',
      recordId: formData.id,
      fileName: `Manual_${formData.id}`,
      fields: [
        { label: 'Manual Title', value: formData.manualTitle },
        { label: 'Type', value: formData.manualType },
        { label: 'Version', value: formData.version },
        { label: 'Department', value: formData.department },
        { label: 'Coordinator', value: formData.responsiblePerson },
        { label: 'Approver', value: formData.approvalAuthority },
        { label: 'Last Review', value: formData.lastReviewDate },
        { label: 'Next Review', value: formData.nextReviewDate },
        { label: 'Status', value: formData.status },
        { label: 'ISO Clauses', value: formData.isoClauses, fullWidth: true },
        { label: 'Content Summary', value: formData.summary, fullWidth: true }
      ],
        ...
        <div className="flex items-center gap-3">
          {isReadOnly ? (
             <>
               ...
               <button type="button" onClick={() => onNavigate('quality-manuals-form', { mode: 'edit', data: formData })} className="btn btn-ghost border border-border-main flex items-center gap-2">
                  <Edit2 className="w-4 h-4" /> Modify Manual
               </button>
             </>
          ) : (
            <>
              <button type="button" onClick={() => onNavigate('quality-manuals')} className="btn btn-ghost px-6">Cancel</button>
